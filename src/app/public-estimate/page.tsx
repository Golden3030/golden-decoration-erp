...
import { Printer, Sparkles, ShieldCheck, Check, Cpu, Layers, Search } from "lucide-react";
...
function PublicEstimateContent() {
  ...
  return (
    <div className="min-h-screen bg-[#020B1C] text-white flex flex-col items-center justify-start p-4 md:p-8 font-sans" dir="rtl">
      ...
      <div className="w-full max-w-3xl bg-[#07132a] border border-[#1f2d4d] rounded-3xl p-6 md:p-8 shadow-2xl mb-8 space-y-4 print:hidden">
        <h3 className="text-lg md:text-xl font-black text-[#D4AF37] flex items-center gap-2">
          <span>🔍</span>
        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row items-center gap-3">
          <input
            ...
            className="w-full h-12 rounded-xl bg-[#020B1C] border border-[#1f2d4d] text-white px-4 outline-none focus:border-[#D4AF37] font-mono font-bold text-center placeholder-gray-600"
          />
          <button
            type="submit"
            className="w-full sm:w-auto h-12 px-8 bg-gradient-to-r from-[#D4AF37] to-[#F0E6D2] text-black font-black rounded-xl text-sm transition hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
          >
            استعلم الآن
          </button>
        </form>
      ...
      {loading ? (
        ...
        <div className="w-full max-w-5xl space-y-6">
          ...
          <div className="bg-[#07132a] border-2 border-[#D4AF37]/50 rounded-[2rem] p-6 shadow-2xl relative overflow-hidden w-full text-right print:hidden">
          ...
          <div className="flex justify-end pr-2 select-none print:hidden">
            <button
              ...
              className="bg-gradient-to-r from-[#D4AF37] to-[#F0E6D2] text-black hover:scale-105 active:scale-95 px-8 py-3.5 rounded-full font-black text-xs transition cursor-pointer flex items-center gap-2 shadow-lg shadow-[#D4AF37]/25"
            >
          ...
          <div className="bg-white rounded-3xl border-2 border-[#D4AF37] shadow-[0_0_60px_rgba(212,175,55,0.25)] p-12 text-[#0B1B38] print:border-0 print:p-0 print:shadow-none" style={{ fontFamily: "Cairo, Arial, sans-serif" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 text-xs font-bold">
              <div className="space-y-3">
                ...
                <div className="border border-gray-200 rounded-xl p-3.5 bg-gray-50/50 flex flex-col justify-between">
                  <span className="text-gray-400 text-[10px] block mb-1">مستوى التشطيب</span>
            ...
            <div className="border border-gray-200 rounded-2xl overflow-hidden mb-8 shadow-sm">
              <table className="w-full border-collapse text-xs text-right">
                <thead>
                  <tr className="bg-[#0B1B38] text-white">
                    <th className="p-3.5 border-b border-gray-200 text-center w-12 font-black">م</th>
                    <th className="p-3.5 border-b border-gray-200 font-black w-48">البند ومجال الأعمال</th>
                    <th className="p-3.5 border-b border-gray-200 font-black">تفصيل ووصف التوريد والتركيب الفني المعتمد للمشروع</th>
                    <th className="p-3.5 border-b border-gray-200 text-center w-16 font-black">الوحدة</th>
                    <th className="p-3.5 border-b border-gray-200 text-center w-16 font-black">الكمية</th>
                    <th className="p-3.5 border-b border-gray-200 text-center w-24 font-black">سعر الوحدة</th>
                    <th className="p-3.5 border-b border-gray-200 text-center w-28 font-black">الإجمالي (ج.م)</th>
                  </tr>
                </thead>
                <tbody>
                  {printItems.map((item: any, idx: number) => {
                    ...
                    return (
                      <tr key={item.id || idx} className="border-b border-gray-100 even:bg-gray-50/60 hover:bg-gray-50 transition">
                        <td className="p-3.5 text-center font-mono text-gray-400 font-bold">{idx + 1}</td>
                        <td className="p-3.5 font-black text-[#0B1B38]">
                          <div className="flex items-center gap-2">
                            <span dangerouslySetInnerHTML={{ __html: getCategoryIconSvg(item.category) }} />
                            <span>{categoryNames[item.category] || item.category || "—"}</span>
                          </div>
                        </td>
                        <td className="p-3.5 text-gray-600 font-bold leading-relaxed">{item.description || item.name}</td>
                        <td className="p-3.5 text-center text-[#0B1B38]">{item.unit || "م²"}</td>
                        <td className="p-3.5 text-center font-mono font-black text-[#0B1B38]">{qty.toLocaleString('en-US')}</td>
                        <td className="p-3.5 text-center font-mono font-bold text-gray-500">{unitPrice.toLocaleString('en-US')}</td>
                        <td className="p-3.5 text-center font-mono font-black text-[#0B1B38]">{rowTotal.toLocaleString('en-US')}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        !loading && (
          /* 🌟 الحالة الافتراضية قبل الاستعلام (تم تعديل الهوية كلياً وتمرير المزايا والواتس اب والمنزلق المحلي) */
          <div className="w-full max-w-5xl space-y-8 animate-fade-in mt-4">
            
            {/* 1. مديول شريط المقارنة التفاعلي (Before & After Slider) - يعتمد على المسارات المحلية لجهازك */}
            <div className="bg-[#07132a] border-2 border-[#D4AF37]/35 rounded-[2rem] p-6 shadow-[0_0_40px_rgba(212,175,55,0.12)] space-y-4 text-right">
              ...
              <div className="relative w-full h-[280px] md:h-[350px] rounded-3xl overflow-hidden border border-[#D4AF37]/30 select-none shadow-2xl">
                ...
                {/* 
                  💡 كود تشغيل الفيديو التكراري الصامت بدلاً من الصور (إذا أردت تفعيله مستقبلاً):
            ...
            <div className="bg-[#07132a] border border-[#1f2d4d] rounded-3xl p-6 shadow-2xl space-y-6">
              <div className="text-right border-b border-[#1f2d4d] pb-3 select-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-right">
                {goldenAdvantages.map((adv, idx) => (
                  <div key={idx} className="p-4 rounded-2xl bg-[#020B1C]/50 border border-[#1f2d4d] hover:border-[#D4AF37]/30 transition-all duration-300 flex flex-col justify-between space-y-2 h-36">
              ...
              <div className="bg-[#020B1C]/80 border-2 border-emerald-500/20 p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-inner">
                ...
                <a
                  ...
                  className="px-6 py-3.5 rounded-full bg-[#10b981] hover:bg-emerald-500 text-[#020B1C] font-black text-xs md:text-sm transition-all duration-200 hover:scale-[1.03] active:scale-[0.97] cursor-pointer shadow-lg shadow-emerald-500/20 flex items-center gap-2 whitespace-nowrap"
                >
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#020B1C] flex items-center justify-center text-[#D4AF37] font-black text-lg animate-pulse">
        جاري تهيئة البوابة الفاخرة للعملاء...
      </div>
    }>
