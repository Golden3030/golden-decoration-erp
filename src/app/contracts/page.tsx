...
import { Trash2, FileText, Search, Sparkles, Loader2 } from "lucide-react";
...
export default function ContractsArchivePage() {
  ...
  return (
    <main className="min-h-screen bg-[#020B1C] relative overflow-hidden" dir="rtl">
      ...
      {/* 🛠️ جدار الحماية البصري المعتمد لتوحيد شريط التمرير بالأسهم والجدول المذهب الموحد وفرض خط Alexandria */}
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *:not(code, pre, .font-mono, [class*="font-mono"]) {
        ...
        tbody td, td {
          ...
          text-align: right !important;
          border-bottom: 1px solid rgba(31, 45, 77, 0.2) !important;
          padding: 12px 14px !important;
        }
      `}} />
      ...
      <section className="w-full lg:pr-56 min-h-screen flex flex-col z-10 relative">
        ...
        <div className="p-4 md:p-8 space-y-6 text-right select-none animate-fade-in">
          ...
          <div className="border-b border-[#D4AF37]/20 pb-5">
            {/* تصغير حجم العنوان لتفادي التداخل ليكون متوافقاً مع الدستور الجمالي الموحد */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* نموذج إضافة وتوثيق المستندات مدمج بإطار شفاف رقيق وموحد */}
            <div className="bg-[#07132a] border border-[#D4AF37] rounded-[2rem] p-6 space-y-5 h-fit shadow-2xl relative overflow-hidden">
              ...
              <h3 className="text-[#D4AF37] font-black text-xs md:text-sm border-b border-[#D4AF37]/20 pb-3 flex items-center gap-2">
              ...
              <div className="space-y-4 font-semibold text-xs">
                ...
                <div>
                  <label className="block text-[#D4AF37] font-bold text-[10px] mb-1">اسم الوثيقة / العقد بالتفصيل *</label>
                  <input
                    type="text"
                    placeholder="مثال: عقد تشطيب شقة التجمع عميل/ ناصر"
                    value={cTitle}
                    onChange={(e) => setCTitle(e.target.value)}
                    className="w-full h-11 rounded-xl bg-[#020B1C] border border-[#1f2d4d] text-white px-4 outline-none focus:border-[#D4AF37] placeholder:text-slate-600 transition-all font-medium text-xs"
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[#D4AF37] mb-1 font-bold text-[10px] whitespace-nowrap">نوع وتصنيف التعاقد *</label>
                    <select
                      value={cType}
                  ...
                  <div>
                    <label className="block text-[#D4AF37] mb-1 font-bold text-[10px] whitespace-nowrap">اسم الطرف الثاني للتعاقد *</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-[#D4AF37] mb-1 font-bold text-[10px]">📁 رابط الملف المرفوع بـ (Google Drive) *</label>
                  <input
                    type="url"
                    placeholder="https://drive.google.com/file/d/..."
                    value={cUrl}
                    onChange={(e) => setCUrl(e.target.value)}
                    className="w-full h-11 rounded-xl bg-[#020B1C] border border-[#1f2d4d] text-emerald-400 px-4 outline-none font-mono focus:border-[#D4AF37] placeholder:text-slate-600 transition-all text-xs"
                  />
                </div>

                <div>
                  <label className="block text-[#D4AF37] mb-1 font-bold text-[10px]">ملاحظات وشروط إضافية للمستند</label>
                  <textarea
                    placeholder="شروط التعاقد، المواعيد، أو أي تفاصيل هامة..."
                    value={cNotes}
                    onChange={(e) => setCNotes(e.target.value)}
                    className="w-full h-20 bg-[#020B1C] border border-[#1f2d4d] text-slate-200 p-3 rounded-xl outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/30 transition-all placeholder-slate-600 resize-none font-medium text-xs"
                  />
                </div>

                {/* زر الإضافة السحابية المطور بالكبسولة المذهبة الفاخرة للشركة والتولتيب */}
                <div className="relative group w-full">
                  <button
                    onClick={handleAddContract}
                    disabled={saving}
                    className="w-full h-11 bg-gradient-to-r from-[#C9A45D] via-[#F0E6D2] to-[#D4AF37] text-[#020B1C] font-black rounded-full text-xs transition-all duration-300 shadow-lg hover:shadow-[#D4AF37]/20 disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer hover:scale-[1.01] active:scale-[0.99]"
                  >
                    {saving ? <Loader2 className="animate-spin w-4 h-4 text-[#020B1C]" /> : "💾 توثيق وأرشفة المستند"}
                  </button>
                  <div className="absolute bottom-full mb-2.5 right-1/2 translate-x-1/2 hidden group-hover:flex flex-col items-center pointer-events-none z-50 animate-fade-in whitespace-nowrap">
                    <div className="bg-[#07132a] border border-[#D4AF37] text-[#F0E6D2] text-[10px] font-black py-1.5 px-3 rounded-lg shadow-2xl relative">
            ...
            {/* كشف الأرشيف السحابي الموحد مدمج بالإطارات شبه الشفافة */}
            <div className="lg:col-span-2 space-y-4">
              <div className="bg-[#07132a] border border-[#D4AF37] rounded-[2rem] overflow-hidden shadow-2xl flex flex-col relative w-full">
                ...
                <div className="p-4 border-b border-[#D4AF37]/20 bg-[#0b1b3d]/60 flex flex-col sm:flex-row justify-between items-center gap-4 select-none">
                  {/* تحويل عنوان كشف العقود للون البني البرونزي المعتمد `#A17A4C` */}
                  <h3 className="text-[#D4AF37] font-black text-xs md:text-sm flex items-center gap-2">
                    <FileText size={18} />
                    <span>كشوف العقود والوثائق المؤرشفة بقاعدة بيانات المنظومة ({filteredContracts.length})</span>
                  </h3>

                  {/* شريط البحث المذهب المطور */}
                ...
                {/* تم تغليف الجدول بـ Scrollbar رأسي مذهب محدد السجلات وحجب الأفقي */}
                    ...
                    <table className="w-full text-right table-auto">
                      ...
                      <tbody className="divide-y divide-[#1f2d4d]/60">
                        {filteredContracts.map((doc) => (
                          <tr key={doc.id} className="hover:bg-[#0B1B38] transition duration-150 whitespace-nowrap cursor-default">
                            <td className="p-4 font-mono text-[#D4AF37] font-black text-xs md:text-sm">{doc.code}</td>
                            <td className="p-4 font-black text-slate-100 text-xs md:text-sm">{doc.title}</td>
                            <td className="p-4">
                              <span className="bg-blue-500/10 text-blue-400 px-3 py-1 rounded-full text-xs font-black border border-blue-500/20">
                                {doc.contract_type}
                              </span>
                            </td>
                            <td className="p-4 text-slate-300 font-bold text-xs md:text-sm">{doc.party_name}</td>
                            <td className="p-4 font-mono text-slate-400 text-xs md:text-sm">{formatArchiveDate(doc.created_at)}</td>
                            <td className="p-4 text-center">
                              ...
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 text-emerald-400 hover:text-emerald-300 hover:underline font-black text-xs transition-all"
                              >
                                🔗 فتح بـ Google Drive
                              </a>
                            </td>
                            <td className="p-4 text-center">
                              {/* زر الحذف الفخم مع التولتيب وحشوة النقر التفاعلية */}
                              <div className="relative group inline-block">
                                <button
                                  onClick={() => handleDeleteContract(doc.id, doc.title)}
